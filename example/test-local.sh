...
function send {
  ...
  /usr/local/bin/irohac --hostname=${IROHA_HOST} --account_id=${CREATOR_ID} $*
}

if [ ! -d ~/.irohac ]; then
  mkdir ~/.irohac
  cp admin@test* ~/.irohac
else
  rm -f ~/.irohac/${USER1_ID}* ~/.irohac/${USER2_ID}*
fi

if [ ! -d block_store ]; then
  mkdir block_store
else
  sudo chown 1000:1000 block_store
fi
